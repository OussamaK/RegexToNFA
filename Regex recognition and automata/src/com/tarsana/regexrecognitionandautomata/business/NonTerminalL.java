...
public class NonTerminalL {

	/**
	 * Indicates whether this non terminal has generated an or operation.
	 */
	Boolean or;
	...
	public NonTerminalL() {
		...
		// this non terminal is producing an OR operation.
		if (Context.getCurrentSymbol().equals("+")) {
			Context.accept("+");
			this.or = true;
			NonTerminalT T = new NonTerminalT();
			if (T.getListeRelations().isEmpty()) {
				Context.raiseError(2);
			}

			// if the right hand side of the operation is missing , raise an
			// error.
			else {
				NonTerminalL L = new NonTerminalL();
				// if there is a recursive call to L that generates an OR
				...
				if (L.getOr()) {
					...
					for (Accepteur accepteur : T.getListeAccepteurs()) {
						...
						this.listeRelations.add(new Triplet(T.getMax()
								+ L.getMax() + 1, accepteur.getSymbol(),
								accepteur.getState()));
					}
			...
			// this non terminal is not producing any OR operations.
			this.or = false;
		}
	}
}
