...
public class NonTerminalD {

	/**
	 * indicates whether this non terminal generated an and symbol.
	 */
	private Boolean and;
	...
	public NonTerminalD() {
		...
		// this non terminal is generating an AND operation.
		if (Context.getCurrentSymbol().equals(".")) {
			...
			// the right hand side of this operation is null , error.
			if (M.getListeRelations().isEmpty()) {
				Context.raiseError(1);
			} else {
				// the recursive call to D resulted in an other AND operation.
				...
				if (D.getAnd()) {
					...
					for (Accepteur accepteurD : D.getListeAccepteurs()) {
						...
						for (Accepteur accepteurM : M.getListeAccepteurs()) {
							this.listeRelations.add(new Triplet(accepteurM
									.getState(), accepteurD.getSymbol(),
									accepteurD.getState()));
						}
					}
				...
				// the recursive call did not produce any more AND operations.
